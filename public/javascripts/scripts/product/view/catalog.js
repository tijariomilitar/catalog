...
Product.view.catalog.filter = async (products, pagination) => {
	...
	for(let i in products){
		if(products[i].product_id){
			html += "<div class='box b4 container ground pointer box-hover padding-10 margin-top-5 border' onclick='window.location.href=`/product/show/"+products[i].product_id+"`'>";
		} else {
			html += "<div class='box b4 container ground pointer box-hover padding-10 margin-top-5 border' onclick='window.location.href=`/product/package/show/"+products[i].id+"`'>";
		};
		if(products[i].image){
		...
		html += `<div class="mobile-box b3-4 avant-garde center em09 padding-10 bold" style="color:#323232;">`+products[i].name+` - `+products[i].color+`</div>`;
	...
	for (let i = pagination.page * pagination.pageSize; i < products.length && i < (pagination.page + 1) * pagination.pageSize; i++){
		if(products[i].product_id){
			...
			html += `<div class="box b3 ground padding-10 margin-top-10 pointer shadow-hover" onclick="window.location.href='/product/package/show/`+products[i].id+`'">`;
		};
